...
function processSum(numberlist, callback) {
  const sum = numberlist.reduce((accumulator, n) => {
...
function getFullNames(arr) {
  let names = [];
  arr.forEach(arr => names.push(`${arr.last_name}, ${arr.first_name}`));
  return names;
}
...
function firstNamesAllCaps(arr) {
  let firstNames = arr.map(arr => arr.first_name.toUpperCase());
  return firstNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let tShirts = runners.filter(item => {
...
function tallyUpDonations(runners) {
  const donate = runners.reduce((accumulator, n) => {
    return accumulator + n.donation;
  }, 0);
  return donate;
}
